...
class AuthRepository {
  ...
  Future<void> saveUserLocally(AppUser user, String pin) async {
  ...
  Future<AppUser> register({
    ...
    // This method might be deprecated in favor of saveUserLocally
    final user = AppUser(
      id: const Uuid().v4(),
      name: name,
      phone: phone,
    ...
    await saveUserLocally(user, pin);
    return user;
  }
  ...
  Future<AppUser> login({
    ...
    // For local demo, we check against the stored user
    if (user == null || user.phone != phone || !verifyPin(pin)) {
  ...
  Future<void> logout() async {
    ...
    await _box.delete(AppConstants.kCurrentUser);
  }
}
