  ...
  static WeatherSnapshot? fromMap(Map raw) {
    try {
      return WeatherSnapshot(
        ...
        fetchedAt:
            DateTime.tryParse(raw['fetchedAt']?.toString() ?? '') ?? DateTime.now(),
...
class WeatherService {
  ...
  /// Fetch fresh weather. Uses cache if fresh (< 6 h), otherwise hits the
  /// Open-Meteo API. On network failure, falls back to the last cached value.
  Future<WeatherSnapshot> fetch({bool force = false}) async {
    final prior = cached();
    if (!force && prior != null && DateTime.now().difference(prior.fetchedAt) < _cacheTtl) {
      return prior;
    }

    LatLng pos = FakeData.defaultCenter;
    try {
      final here = await _location.current();
      if (here != null) pos = here;
    } catch (_) {
      // permissions / sensor issues → fall back to default center.
    }
    ...
    try {
      ...
      final snapshot = WeatherSnapshot(
        ...
        condition: _conditionLabel((current['weather_code'] as num?)?.toInt() ?? 0),
