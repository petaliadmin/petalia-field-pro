...
class _RoutePlannerScreenState extends ConsumerState<RoutePlannerScreen> {
  ...
  void _optimize() {
    ...
    final order =
        GeoUtils.nearestNeighborRoute(FakeData.defaultCenter, centroids);
    setState(() {
...
class _RouteMapView extends StatelessWidget {
  const _RouteMapView({required this.parcels});
  ...
  Widget build(BuildContext context) {
    ...
    final allPoints = centroids.toList()..insert(0, FakeData.defaultCenter);
    final bounds = LatLngBounds.fromPoints(allPoints);

    // Total distance
    double totalKm = 0;
    LatLng cursor = FakeData.defaultCenter;
        ...
        FlutterMap(
          ...
          children: [
            ...
            PolylineLayer(
              polylines: [
                Polyline(
                  points: [FakeData.defaultCenter, ...centroids],
                  strokeWidth: 3,
